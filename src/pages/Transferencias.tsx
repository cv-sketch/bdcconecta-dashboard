...
const EMPTY_FORM = {
  fromWalletId: '', toAddress: '', toCuit: '',
  amount: '', concept: 'VAR', description: ''
}

export default function Transferencias() {
  const { wallets, clientes, transferencias, addTransferencia, updateSaldo } = useStore()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(monto) || monto <= 0) { setMsg({tipo:'err',texto:'El monto debe ser mayor a 0'}); return }
    if (monto > walletOrigen.saldo) { setMsg({tipo:'err',texto:'Saldo insuficiente. Disponible: $' + walletOrigen.saldo.toLocaleString('es-AR')}); return }
    setLoading(true); setMsg(null)
    try {
      const cliente = getCliente(walletOrigen.clienteId)
      const oid = 'trx-' + Date.now()
      const res = await bdcService.enviarTransferencia({
        originId: oid, fromAddress: walletOrigen.cvu, fromCuit: cliente?.cuit || '',
        toAddress: form.toAddress, toCuit: form.toCuit, amount: monto,
        concept: form.concept, description: form.description
      })
      if (res.ok) {
        addTransferencia({
          originId: oid, fromWalletId: walletOrigen.id, fromCvu: walletOrigen.cvu,
          toAddress: form.toAddress, toCuit: form.toCuit, amount: monto,
          concept: form.concept, description: form.description,
          estado: 'completada', coelsaId: res.coelsaId, createdAt: new Date().toISOString()
        })
        updateSaldo(walletOrigen.id, monto, 'debito')
        setMsg({tipo:'ok', texto:'Transferencia enviada! ID Coelsa: ' + res.coelsaId})
        setForm(EMPTY_FORM); setTab('historial')
      } else {
        addTransferencia({
          originId: oid, fromWalletId: walletOrigen.id, fromCvu: walletOrigen.cvu,
          toAddress: form.toAddress, toCuit: form.toCuit, amount: monto,
          concept: form.concept, description: form.description,
          estado: 'fallida', createdAt: new Date().toISOString()
        })
              ...
              <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:16}}>
                <div>
                  ...
                  <select value={form.fromWalletId} onChange={e=>setForm(p=>({...p,fromWalletId:e.target.value}))} required style={{width:'100%',background:'#111827',border:'1px solid #374151',borderRadius:6,padding:'10px 12px',color:'#fff',fontSize:14,boxSizing:'border-box'}}>
                    ...
                    {walletsActivas.map(w=>{
                      ...
                      return <option key={w.id} value={w.id}>{c?c.nombre+' '+c.apellido:'Sin titular'} — ${w.saldo.toLocaleString('es-AR')} — CVU: {w.cvu.slice(-8)}</option>
                    })}
                  </select>
                </div>
                {walletOrigen && (
                ...
                <div>
                  ...
                  <select value={form.concept} onChange={e=>setForm(p=>({...p,concept:e.target.value}))} style={{width:'100%',background:'#111827',border:'1px solid #374151',borderRadius:6,padding:'10px 12px',color:'#fff',fontSize:14,boxSizing:'border-box'}}>
                    {[['VAR','Varios'],['ALQ','Alquiler'],['CUO','Cuota'],['HAB','Haberes'],['HON','Honorarios'],['FAC','Factura'],['PRE','Préstamo']].map(([v,l])=><option key={v} value={v}>{v} - {l}</option>)}
                  </select>
                </div>
                <div>
              ...
              {transferencias.length===0 ? (
                ...
                return (
                  <tr key={t.id} style={{borderBottom:'1px solid #374151',background:i%2===0?'transparent':'#111827'}}>
                    ...
                    <td style={{padding:'12px 16px',color:'#D1D5DB',fontFamily:'monospace',fontSize:12}}>{t.fromCvu.slice(-10)}</td>
